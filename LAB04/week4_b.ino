...
for(int i=0;i<75;i++){
  for(int j=0;j<75-i;j++){
    aut[i] = aut[i] + first[j]*first[j+i];
  }
  aut[i]=aut[i]/m;
  
}
for(int i=0;i<75;i++){
  if(aut[i] >0 && aut[i+1] <=0 && c==0){
    k=i;
    c++;
    max2=0;
  }
